...
public class EventTeleporter implements Runnable
{
	 ...
	 * @param adminRemove
	 */
	public EventTeleporter(L2PcInstance playerInstance, int[] coordinates, boolean fastSchedule, boolean adminRemove)
	{
		_playerInstance = playerInstance;
		_coordinates = coordinates;
		_adminRemove = adminRemove;
		
		long delay = (EventEngine.isStarted() ? Config.TVT_EVENT_RESPAWN_TELEPORT_DELAY : Config.TVT_EVENT_START_LEAVE_TELEPORT_DELAY) * 1000;
	...
	public void run()
	{
		...
		int EventsInstance = EventEngine.getEventInstance();
		if (EventsInstance != 0)
		{
			if (EventEngine.isStarted() && !_adminRemove)
			{
				_playerInstance.setInstanceId(EventsInstance);
			}
			else
			{
		...
		if (EventEngine.isStarted() && !_adminRemove)
		{
			int teamId = EventEngine.getParticipantTeamId(_playerInstance.getObjectId()) + 1;
			switch (teamId)
			{
				case 0:
